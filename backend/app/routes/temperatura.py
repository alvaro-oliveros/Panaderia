...
def crear_lectura_temperatura(temperatura: schemas.TemperaturaCreate, db: Session = Depends(get_db)):
    nueva = models.Temperatura(**temperatura.dict())
    db.add(nueva)
    db.commit()
    db.refresh(nueva)
    return nueva
...
@router.get("/")
def obtener_temperaturas(user_id: int = None, sensor_id: int = None, db: Session = Depends(get_db)):
    ...
    temperaturas = query.order_by(models.Temperatura.fecha.desc()).all()
    return temperaturas
